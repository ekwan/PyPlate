...
def test_transfer_between_containers(solution1, solution2, water, salt, dmso, sodium_sulfate):
    ...
    solution1_volume = 100 + salt.convert_quantity('50 mmol', 'mL')
    solution2_volume = 100 + sodium_sulfate.convert_quantity('50 mmol', 'mL')
    ...
    assert solution3.contents[water] == pytest.approx(water.convert_quantity('90 mL', config.moles_storage_unit))
    assert solution3.contents[salt] == pytest.approx(salt.convert_quantity('45 mmol', config.moles_storage_unit))
    assert solution4.contents[water] == pytest.approx(water.convert_quantity('10 mL', config.moles_storage_unit))
    assert solution4.contents[salt] == pytest.approx(salt.convert_quantity('5 mmol', config.moles_storage_unit))
